	...
	<div class="layuimini-container">
		<div class="layuimini-main">
			...
			<c:if test="${sessionScope.role==2 }">
				...
				<a class="layui-btn layui-btn-xs data-count-edit" lay-event="baom">选课</a>            	
			</c:if>
        	</script>
		</div>
	</div>
...
<div class="layuimini-container" id="popView" style="display: none;">
	...
	editor2.create();
	 ...
	 layui.use(['form', 'table','laydate','upload','rate'], function () {
        ...
        table.render({
            elem: '#currentTableId',
            url: '<%=Const.ROOT%>kecheng/list',
            cols: [[
				{field:'id',hide:true,width: 0},
				{width: 100, templet:'#no',title: 'NO', sort: true},
				 {templet: '#tid', width: 100, title: '教师'},
				 {field: 'type', width: 120, title: '课程类型'},
				 {field: 'name', width: 250, title: '标题'},
                {field: 'content', width: 250, title: '课程介绍'},
                {field: 'optime', width: 150, title: '开课时间'},
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            var data = obj.data;
            if (obj.event === 'view') {
            	$.getJSON('<%=Const.ROOT%>kecheng/json',{'id':obj.data.id},function(data) {
            		$("#popView #vname").html("").html(data.name);
            		$("#popView #vtype").html("").html(data.type);
            		$("#popView #vcname").html("").html(data.cname);
               ...
               setFormValue(obj,data);  
            	...
            	layer.confirm('是否审核通过？', function (index) {
                    ...
                    $.getJSON('<%=Const.ROOT%>kecheng/update',{'id':obj.data.id,'status':'已审核'},function(msg) {
  						//console.log(msg);
  						if(msg.code=="0"){
            ...
            }else if (obj.event === 'baom') {
            	layer.confirm('是否选课？', function (index) {
                    //console.log(obj.data);
                    var url="<%=Const.ROOT%>xuanke/add";
                    if(obj.data.type=="实践课"){
                    	url="<%=Const.ROOT%>shijian/add";
                    }
                    $.getJSON(url,{'kid':obj.data.id,'uid':'${sessionScope.users.id}'},function(msg) {
  						//console.log(msg);
  						if(msg.code=="0"){
              ...
              //表单初始赋值
               ...
               setFormValue(obj,data);    
                ...
                layer.confirm('真的删除行么', function (index) {
                    //console.log(obj.data);
                    $.getJSON('<%=Const.ROOT%>kecheng/delete',{'id':obj.data.id},function(msg) {
  						//console.log(msg);
  						if(msg.code=="0"){
  							layer.msg(msg.msg, { icon: 6, time: 800},function(){
  								layer.close(index);
        ...
        form.on('submit(add)', function (data) {
        	var url="<%=Const.ROOT%>kecheng/add";
            $.post(url,data.field,function(data){
            	...
            	if(data.code==1){//5错误，6正常
            		...
            		layer.msg(data.msg,{"icon":6,time:2000},function(){
            			location.href="<%=Const.ROOT%>kecheng.jsp";
            		});
            	}	
            });
